...
export default function MessagingPage(): JSX.Element {
  ...
  const handleSendMessage = async () => {
    if (!newMessage || !chatId || !senderId) return;
              ...
              <header className="w-full border-primary/30 border-b">
                <div className="w-[95%] mx-auto flex justify-between items-center py-5">
                  <div className="flex items-center gap-4">
                    {riderDetails.avatar && (
                      <Image src={riderDetails.avatar} alt="rider" className="rounded-full w-12 h-12" />
                    )}
                    <div>
              ...
              <div className="space-y-3 mt-6 w-[95%] mx-auto">
                <div className="flex gap-3 overflow-x-auto no-scrollbar">
                  {QUICK_CHAT_MESSAGES.map((message) => (
                    <QuickButton key={message} text={message} />
                  ))}
                </div>
                <div className="bg-background border border-foreground/20 rounded-xl overflow-clip divide-y divide-foreground/20">
                  <div className="flex px-4">
                    ...
                    />
                    <button className="text-primary " onClick={handleSendMessage}>
...
const QuickButton = memo(({ text }: QuickButtonProps): JSX.Element => {
  const sendQuickMessage = useCallback(async () => {
    // Send Quick Message to rider (you can implement later)
  }, []);

  return (
    <Button variant="secondary" className="min-w-[10rem]" onClick={sendQuickMessage}>
      {text}
    </Button>
  );
});

function Message({ sender, content }: { sender: string; content: string }): JSX.Element {
  return (
